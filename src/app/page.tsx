...
export default function HomePage() {
  ...
  useEffect(() => {
    if (isMounted) { // Only save to localStorage if client is mounted and tasks have changed
  ...
  const handleUpdateTaskText = useCallback((id: string, newText: string) => {
    ...
    } else { // Should ideally not happen if update is triggered from existing task
        toast({
          title: "Task updated!",
  ...
  const handleGetTaskInsight = useCallback(async (task: Task) => {
    ...
    setInsightData(null); // Clear previous insight data
    try {
      ...
      setInsightData(null); // Ensure data is null on error
      toast({
  ...
  }, [tasks, toast]); // Added tasks to dependency array
  ...
  const handleAddSubTasksToList = useCallback((subTaskTexts: string[], parentTaskText: string) => {
    ...
    setAiMessage(successMessage); // Also update the AI assistant message
  }, [toast]);
